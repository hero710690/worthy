...
| View coupling | Projection shows **only in INVESTMENTS ONLY view** (`viewMode === 'invested'`); hidden in ALL ASSETS. Seeded from `invest_value`. Controls (rate input, toggle) also only appear in INVESTMENTS ONLY + ALL range. |
| Contribution injection | Instead of injecting each contribution on its actual date, compute a **flat monthly average** = (sum of investable Recurring + LumpSum + Initialization contributions) ÷ (window months). Inject that amount each step, prorated by the step's days (`monthlyAvg × step_days / 30.44`), applied after compounding. |
