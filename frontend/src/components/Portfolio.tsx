...
export const Portfolio: React.FC = () => {
  ...
  const handleExportCSV = () => {
    if (!allocation || !assets) return;
    const baseCurrency = user?.base_currency || 'USD';
    const timestamp = new Date().toISOString().slice(0, 19).replace(/[T:]/g, '-');
    const headers = ['Symbol', 'Type', 'Shares', 'Avg Cost Basis', 'Currency', 'Current Price', `Market Value (${baseCurrency})`, `Gain/Loss (${baseCurrency})`, 'Gain/Loss %', 'Allocation %'];
    const rows = allocation.byAsset.map(a => {
      const asset = assets.find(x => x.ticker_symbol === a.symbol);
      return [
        a.symbol,
        asset?.asset_type || '',
        asset?.total_shares?.toFixed(4) || '',
        asset?.average_cost_basis?.toFixed(2) || '',
        asset?.currency || '',
        a.currentPrice?.toFixed(2) || '',
        a.value?.toFixed(2) || '',
        a.gainLoss?.toFixed(2) || '',
    ...
    link.href = url;
    link.download = `portfolio-snapshot-${timestamp}.csv`;
    link.click();
    ...
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1400, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="Export portfolio as CSV">
            <Button
              variant="outlined"
